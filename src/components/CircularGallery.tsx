...
/**
 * CircularGallery — 基于 react-bits (MIT) 的弧形 3D 画廊组件
 * 使用 OGL (WebGL) 渲染，支持鼠标拖拽 / 滚轮 / 触摸滑动
 * 改造为 TypeScript + Next.js 兼容版本
 */
...
async function loadFontFromStylesheet(url: string) {
  ...
  for (const block of faceBlocks) {
    ...
    const urlMatch = block.match(/url\(\s*['"]?([^'")\s]+)['"]?\s*\)/);
...
class Media {
  ...
  update(scroll: { current: number; last: number }, _direction: string) {
    ...
    if (_direction === "right" && this.isBefore) {
      this.extra -= this.widthTotal;
      this.isBefore = this.isAfter = false;
    }
    if (_direction === "left" && this.isAfter) {
      this.extra += this.widthTotal;
      this.isBefore = this.isAfter = false;
    }
  }
...
class GalleryApp {
  ...
  onWheel(e: WheelEvent) {
    // 支持双向：deltaY > 0 向右滚，deltaY < 0 向左滚，也支持 deltaX 横向滚轮
    const delta = e.deltaY !== 0 ? e.deltaY : e.deltaX;
    this.scroll.target += delta * this.scrollSpeed * 0.004;
